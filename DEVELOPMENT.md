...
# Backend utilities
## 🌍 Translation (Babel)
1. Extract messages and update translations:
```bash
...
2. Translate text from `messages.po`:
```bash
# Example of Spanish translation

#: app/tasks.py:22 app/tasks.py:33
#, python-format (interpolation example)
msgid "Email sent to %(email_to)s"
msgstr "Correu electrònic enviat a %(email_to)s"

#: app/api/routes/auth.py:56
msgid "Incorrect email or passowrd"
...
# Frontend utilities
## 🌍 Translation (ParaglideJS)
Kura uses **ParaglideJS (Inlang message format)** to manage frontend translations in a type-safe and consistent way.

All translations are stored as JSON key-value files per language.

---

## 📁 Language Files

Each language must have the same keys:
...
## 🔤 1. Normal Messages (Static Text)

Static messages are simple key-value pairs without variables.
...
🧩 2. Messages with Interpolation (Dynamic Values)

Interpolation allows you to inject variables into messages using {} placeholders.
...
### Usage in code
You must pass an object with the required variables:
